...
public class RocksDBClient extends DB {
  ...
  private static Boolean lockMemory = false;
    ...
    synchronized(RocksDBClient.class) {
      ...
      if(rocksDb == null) {
        ...
        if (lockMemory) {
          ...
          int LOCK_STATUS = c.syscall(151, 2);
	  if (LOCK_STATUS == 0) {
		LOGGER.info("memory locked for future access");
	  } else {
	  	LOGGER.error("memory locked failed. please check permission.");
	  }
        }
